...
void handler_l(DIR* dir, struct dirent* ent, char* path, int len)
{
	...
	while((ent = readdir(dir)) != NULL)//пока не закончатся входы в каталог или не будет ошибки
	{
		...
		{
			memcpy(path + len, ent -> d_name, strlen(ent -> d_name));// копируем название файла в буфер
...
int main(int argc, char** argv)
{
	...
	switch(argc)
	{
		...
		case 3:
		{
		     ...
		     {
			memcpy(path, argv[2], strlen(argv[2]));// копируем название директории в буфер
			len = strlen(argv[2]);
			dir = opendir(argv[2]);
		     }
		     else if(optind == 1)
